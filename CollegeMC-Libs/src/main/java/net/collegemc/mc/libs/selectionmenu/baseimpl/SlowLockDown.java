...
public class SlowLockDown implements TieDown {
  ...
  public void tieDown(Player player) {
    Objects.requireNonNull(player.getAttribute(Attribute.GENERIC_MOVEMENT_SPEED)).addModifier(this.modifier);
    player.addPotionEffect(this.jumpEffect);
  }

  @Override
  public void release(Player player) {
    Objects.requireNonNull(player.getAttribute(Attribute.GENERIC_MOVEMENT_SPEED)).removeModifier(this.modifier);
