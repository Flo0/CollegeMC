...
public class CraftleManager {
  ...
  public void startCraftle(Location location, Player player) {
    ...
    CraftleDisplay craftleDisplay = new CraftleDisplay(currentId, location.toVector(), location.getDirection(), possibleWords.stream().toList().get(ThreadLocalRandom.current().nextInt(possibleWords.size())));
  ...
  private void loadWords() {
    ...
    possibleWords = bufferedReader.lines().collect(Collectors.toSet());
  }
